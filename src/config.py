...
class Settings(BaseSettings):
    ...
    cleanup_after_hours: int = Field(
        default=24, description="Hours to keep files before cleanup"
    )

    # Queue
    redis_url: str = Field(
        default="redis://redis:6379/0", description="Redis URL for queue"
    )
    queue_name: str = Field(default="pdf_translation", description="Queue name")

    # Database
    database_url: str = Field(
        default="sqlite:///./jobs.db", description="Database URL for job metadata"
    )

    # Translation
    model_name: str = Field(
        default="facebook/nllb-200-3.3B", description="Translation model name"
    )
    model_revision: Optional[str] = Field(
        default="refs/pr/17", description="Model revision"
    )
    use_safetensors: bool = Field(default=True, description="Use safetensors format")
    cpu_load_then_gpu: bool = Field(
        default=True, description="Load on CPU then move to GPU"
    )
    max_input_tokens: int = Field(default=950, description="Max input tokens per chunk")
    max_new_tokens: int = Field(
        default=400, description="Max new tokens per generation"
    )
    ...
    source_language: str = Field(
        default="auto", description="Source language for translation"
    )
    target_language: str = Field(
        default="fr_XX", description="Target language for translation"
    )
