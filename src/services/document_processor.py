...
class DocumentProcessor:
    ...
    def process_pdf(self, input_path: Path, output_dir: Path, 
                   document_type: DocumentType, progress_callback=None) -> Path:
        """
    ...
    def _run_ocr(self, input_path: Path, work_dir: Path, document_type: DocumentType) -> Path:
    ...
    def _run_docling(self, input_path: Path, work_dir: Path) -> Path:
        ...
        cmd = [
            ...
            "--no-ocr",  # We handle OCR separately
        ...
        try:
            ...
            if result.returncode != 0:
                raise DocumentProcessingError(f"Docling conversion failed: {result.stderr}")
            
            # Find the generated markdown file
            md_files = list(output_dir.glob("*.md"))
            if not md_files:
                raise DocumentProcessingError("No markdown file generated by Docling")
            ...
            raise DocumentProcessingError("Docling conversion timed out")
        except FileNotFoundError:
            raise DocumentProcessingError("docling not found - please install it")
