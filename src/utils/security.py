...
def validate_filename(filename: str) -> str:
    ...
    if not any(sanitized.lower().endswith(ext) for ext in ALLOWED_EXTENSIONS):
        raise SecurityError(f"File extension not allowed. Allowed: {', '.join(ALLOWED_EXTENSIONS)}")
...
def validate_upload_file(file: UploadFile) -> Tuple[str, bytes]:
    ...
    try:
        ...
        if file.content_type and file.content_type not in ALLOWED_MIME_TYPES:
            logger.warning(f"Suspicious MIME type: {file.content_type} for file: {sanitized_filename}")
        ...
        logger.info(f"File validation successful: {sanitized_filename} ({len(file_content)} bytes)")
