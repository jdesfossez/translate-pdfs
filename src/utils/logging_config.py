...
class StructuredLogger:
    ...
    def log_job_event(self, job_id: str, event: str, **kwargs):
        ...
        self.logger.info(f"JOB_EVENT job_id={job_id} event={event} {' '.join(f'{k}={v}' for k, v in kwargs.items())}")
    
    def log_api_request(self, method: str, path: str, status_code: int, duration: float, **kwargs):
        """Log API requests with structured data."""
        self.logger.info(f"API_REQUEST method={method} path={path} status={status_code} duration={duration:.3f}s {' '.join(f'{k}={v}' for k, v in kwargs.items())}")
    
    def log_error(self, error_type: str, message: str, **kwargs):
        """Log errors with structured data."""
        self.logger.error(f"ERROR type={error_type} message={message} {' '.join(f'{k}={v}' for k, v in kwargs.items())}")
